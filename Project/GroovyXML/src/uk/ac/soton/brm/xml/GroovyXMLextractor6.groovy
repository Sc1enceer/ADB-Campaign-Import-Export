...
//TODO remove root org from exclude base
	...
	def static final campaignName = "ModuleMarksRelease_FullYear_1213_Continuing"
	...
	//get the root node
	def static root = new XmlParser().parse(new File('dev2_full_260613.xml')) //the XML file to parse
    ...
    static void main(String[] args) {
		...
		//find templateversions associated with comm template
		templateNames.each { template ->
		...
		//TODO This!
		//for each commFieldNames, need to extract the rule name and exclude baseline rules
		//need to get the right fildset / filed combination. two field wiht the same name can exists in seperate fieldsets
		...
		//get ruleConstantXML - but - should I get ruleConstants that are found above, or all rule constants. some rule constants dont seemt o be assosciated with a rule
...
*/
	...
	File outputFile = new File("campaign.xml")
